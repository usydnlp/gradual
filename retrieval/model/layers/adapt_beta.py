...
class ADAPT(nn.Module):
    ...
    def forward(self, value, query, sg_embed):
        ...
        alpha = self.fc_sg(sg_embed).view(
            Bv, Dv // self.groups, 1, 1
        )


        normalized = value * (gammas + 1) + betas + alpha
