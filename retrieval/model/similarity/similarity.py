...
class Similarity(nn.Module):
    ...
    def forward(self, q1_embed, q2_embed, v1_embed, v2_embed, lens, shared=False):
        logger.debug((
            f'Similarity - img_shape: {v1_embed.shape} '
            'cap_shape: {q1_embed.shape}'
        ))

        return self.similarity(q1_embed, q2_embed, v1_embed, v2_embed, lens)

    def forward_shared(self, q1_embed, q2_embed, v1_embed, v2_embed, lens, shared_size=128):
        """
        Compute pairwise i2t image-caption distance with locality sharding
        """

        #img_embed = v1_embed.to(self.device)
        #cap_embed = q1_embed.to(self.device)

        n_im_shard = (len(v1_embed)-1)//shared_size + 1 #2
        n_cap_shard = (len(q1_embed)-1)//shared_size + 1 #8

        print("## ----- [forward_shared]")
        print("n_im_shard: "+str(n_im_shard))
        print("n_cap_shard: "+str(n_cap_shard))
        ...
        d = torch.zeros(len(v1_embed), len(q1_embed)).cpu()
        for i in pbar_fn(n_im_shard):
            im_start = shared_size*i
            im_end = min(shared_size*(i+1), len(v1_embed))
            for j in range(n_cap_shard):
                cap_start = shared_size*j
                cap_end = min(shared_size*(j+1), len(q1_embed))
                im = v1_embed[im_start:im_end]
                s = q1_embed[cap_start:cap_end]
                if q2_embed is not None:
                    q2 = q2_embed[cap_start:cap_end]
                else:
                    q2 = None
                if v2_embed is not None:
                    v2 = v2_embed[im_start:im_end]
                else:
                    v2 = None
                #sg = sg_embed[im_start:im_end]
                l = lens[cap_start:cap_end]
                #print("&*******")
                #print(s.shape)
                #print(im.shape)
                sim = self.forward(s, q2, im, v2, l)
                #print(sim.shape)
...
class AdaptiveEmbeddingT2I(nn.Module):
    ...
    def __init__(
            self, device, q1_size=1024, q2_size=1024, v1_size=1024, v2_size=1024, k=1,
            gamma=10, train_gamma=False, clip_embeddings=True,
            normalization='batchnorm', use_fovea=True, latent_size=None,
        ):
        super().__init__()

        self.device = device

        self.norm = Normalization(v1_size, normalization)

        self.adapt_img = adapt.ADAPT(k, q1_size, q2_size, v1_size, v2_size)

        self.fovea = nn.Identity()
        if use_fovea:
            self.fovea = Fovea(smooth=gamma, train_smooth=train_gamma)

    def forward(self, q1_embed, q2_embed, v1_embed, v2_embed, lens, **kwargs):
        '''
        ...
        '''
        # (B, 1024, T)
        cap_embed = q1_embed.permute(0, 2, 1)
        img_embed = v1_embed.permute(0, 2, 1)
        ...
        for i, cap_tensor in enumerate(cap_embed):
            ...
            q2_repr = None if q2_embed is None else q2_embed[i,:].unsqueeze(0)

            img_output = self.adapt_img(img_embed, v2_embed, cap_repr, q2_repr)
...
class AdaptiveEmbeddingI2T(nn.Module):
    ...
    def __init__(
        ...
        if normalization:
            self.norm = Normalization(q1_size, normalization)

        self.adapt_txt = adapt.ADAPT(k, v1_size, v2_size, q1_size, q2_size)
    ...
    def forward(self, q1_embed, q2_embed, v1_embed, v2_embed, lens, **kwargs):
        ...
        # (B, 1024, T)
        #
        cap_embed = q1_embed.permute(0, 2, 1)[...,:34]
        img_embed = v1_embed.permute(0, 2, 1)
        ...
        for i, img_tensor in enumerate(img_embed):
            ...
            v2_repr= None if v2_embed is None else v2_embed[i,:].unsqueeze(0)

            txt_output = self.adapt_txt(cap_embed, q2_embed, img_vector, v2_repr)
