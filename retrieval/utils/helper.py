...
def save_checkpoint(
        ...
        is_best=False, save_all=False, iteration=0, epoch=0, **kwargs
    ):
    ...
    torch.save(
        obj=state_dict,
        f=os.path.join(outpath, f'checkpoint_{epoch}_{iteration}.pkl'),
    )

    if is_best:
        import shutil
        shutil.copy(
            os.path.join(outpath, f'checkpoint_{epoch}_{iteration}.pkl'),
            os.path.join(outpath, 'best_model.pkl'),
        )

def load_model(path, sg_type, data_dir, txt_pooling, txt_pooling_type, img_pooling,img_pooling_type):
    ...
    model = model.Retrieval_BISG(**model_params, tokenizers=tokenizers, sg_type=sg_type, txt_pooling=txt_pooling, txt_pooling_type=txt_pooling_type, img_pooling=img_pooling,img_pooling_type=img_pooling_type)

    model.initialize_sg_encoders(data_dir)
    #print(checkpoint['args']['dataset']['data_path'])
...
def restore_checkpoint(path, model=None, optimizer=False):
    ...
    if model is None:
        ...
        model = model.Retrieval_BIGSG(**model_params)
